...
export default function ProfileContent({ profileUserId }: { profileUserId?: string }) {
  ...
  const isOwnProfile = !profileUserId || profileUserId === user?.id?.toString();
  ...
  useEffect(() => {
    ...
    if (profileUserId) loadUserProfile();
  ...
  async function loadUserProfile() {
    ...
    setLoading(true);
    try {
      const { data } = await api.get(`/auth/users/${profileUserId}`);
      setProfile(data.user);
      setWatched(data.watched?.items || []);
      setTotalWatched(data.watched?.total || 0);

      if (user && user.id.toString() !== profileUserId) {
        try {
          const { data: fData } = await api.get('/social/friends');
          const isFriend = (fData.friends || []).some((f: { id: string }) => String(f.id) === String(profileUserId));
          if (isFriend) {
            setFriendStatus('friends');
          } else {
            const { data: rData } = await api.get('/social/friends/requests');
            const incoming = (rData.requests || []).find((r: { from: { id: string } }) => String(r.from.id) === String(profileUserId));
      ...
      try {
        const { data: cData } = await api.get(`/profile-comments/${profileUserId}`);
  ...
  async function sendRequest() {
    if (!profileUserId) return;
    try { await api.post('/social/friends/request', { user_id: profileUserId }); setFriendStatus('sent'); } catch {}
  }
  ...
  async function removeFriend() {
    if (!profileUserId) return;
    try { await api.delete(`/social/friends/${profileUserId}`); setFriendStatus('none'); } catch {}
  }
  ...
  async function addComment() {
    if (!commentText.trim() || !profileUserId) return;
    try {
      await api.post(`/profile-comments/${profileUserId}`, { body: commentText.trim() });
      setCommentText('');
      const { data: cData } = await api.get(`/profile-comments/${profileUserId}`);
      setComments(cData.comments || []);
    } catch {}
    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="relative px-8 pb-6">
        <div className="flex items-end gap-6 -mt-14">
          <div className="relative shrink-0 group">
            <div className="w-28 h-28 rounded-full bg-surface2 border-4 border-ink overflow-hidden flex items-center justify-center shadow-xl shadow-black/30">
              {displayAvatar ? (
          ...
          <div className="flex-1 pb-1 min-w-0">
            ...
            {displayCreatedAt && (
              <p className="text-xs text-gray-600 mt-1">
                {new Date(displayCreatedAt).toLocaleDateString('tr-TR', { year: 'numeric', month: 'long', day: 'numeric' })} tarihinden beri üye
              </p>
            )}
          </div>
        ...
        {isOwnProfile ? (
          <div className="mt-4 space-y-3">
            ...
            {saved && <p className="text-xs text-green-400">Değişiklikler kaydedildi.</p>}
          </div>
        ) : displayBio ? (
        ...
        <div className="flex items-center gap-6 mt-5 pt-5 border-t border-white/[0.06]">
          <div className="text-center">
            ...
            <p className="text-[11px] text-muted uppercase tracking-wider">Arkadaş</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-white">{isOwnProfile ? '-' : totalWatched}</p>
            <p className="text-[11px] text-muted uppercase tracking-wider">İzlenen</p>
          </div>
          <div className="text-center">
      ...
      <div className="flex border-b border-white/[0.06] mt-2">
        <button
          ...
          className={`px-6 py-3 text-sm font-semibold transition-colors relative ${activeTab === 'favorites' ? 'text-honey' : 'text-muted hover:text-white'}`}
        >
          Favoriler
        ...
        <button
          ...
          className={`px-6 py-3 text-sm font-semibold transition-colors relative ${activeTab === 'comments' ? 'text-honey' : 'text-muted hover:text-white'}`}
        >
            ...
            {isOwnProfile && (
              <div className="mb-4">
                <Link href="/library" className="text-xs text-honey hover:text-honey-light transition-colors">
                  Kütüphanenden favorilerini seç &rarr;
                </Link>
              </div>
            )}
            {watched.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-sm text-muted">Henüz favori içerik yok</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <span className={`inline-block mt-1 text-[9px] font-bold px-1.5 py-0.5 rounded ${item.type === 'anime' ? 'bg-rose-500/80 text-white' : item.type === 'tv' ? 'bg-emerald-500/80 text-white' : 'bg-honey/80 text-ink'}`}>
                        {item.type === 'tv' ? 'DİZİ' : item.type === 'anime' ? 'ANİME' : 'FİLM'}
                      </span>
                    </div>
                  </Link>
                ))}
          ...
          <div>
            ...
            {comments.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-xs text-gray-600 mt-1">İlk yorumu sen yap!</p>
              </div>
            ) : (
              <div className="space-y-4">
                {comments.map(comment => (
                  <div key={comment.id} className="flex gap-3 group">
                    <button
                      onClick={() => comment.author?.id && router.push(`/profile/${comment.author.id}`)}
                      className="w-9 h-9 rounded-full bg-surface2 border border-white/[0.06] overflow-hidden shrink-0 flex items-center justify-center hover:ring-2 hover:ring-honey/30 transition-all"
                    >
                    ...
                    <div className="flex-1 bg-surface rounded-xl px-4 py-3 border border-white/[0.04]">
                      <div className="flex items-center justify-between">
                        ...
                        {(isOwnProfile || comment.author?.id === user.id) && (
