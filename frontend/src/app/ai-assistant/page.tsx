  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[#c0392b] text-[11px] font-mono uppercase tracking-[0.2em]">AI</span>
      </div>
      ...
      {/* Input Box */}
      <div className="bg-[#141210] border border-white/[0.06] rounded-2xl p-5 mb-5">
        <div className="flex gap-3 items-end">
          ...
          <button
            ...
            className="bg-[#c0392b] hover:bg-[#a93226] text-white px-5 py-3 rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2 shrink-0 font-medium text-sm shadow-lg shadow-[#c0392b]/20"
          >
            <Send size={16} />
      ...
      {response && (
        <div>
          {response.message && (
            <div className="bg-[#c0392b]/5 border-l-4 border-[#c0392b] rounded-r-2xl px-5 py-4 mb-6">
          ...
          <div className="flex flex-col gap-4">
            {response.suggestions?.map((s, i) => (
              <div key={i} className="bg-[#141210] border border-white/[0.06] rounded-2xl p-5 flex gap-5 hover:border-white/15 transition-colors">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    {s.rating && (
                      <span className="text-xs text-yellow-400 font-semibold">⭐ {s.rating}</span>
                    )}
                  </div>
                  {s.genres && <p className="text-xs text-gray-500 mb-2">{s.genres}</p>}
                  <div className="mb-2">
                    <span className="text-[11px] uppercase tracking-wider text-[#c0392b] font-bold">BUNU NEDEN ÖNERİYORUM</span>
                  </div>
                  <p className="text-sm text-gray-300 leading-relaxed">{s.reason}</p>
                  <Link
                    href={`/?q=${encodeURIComponent(s.title)}`}
                    className="inline-flex items-center gap-1 text-xs text-[#c0392b] hover:underline mt-3"
                  >
